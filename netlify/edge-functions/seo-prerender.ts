/**
 * Netlify Edge Function: SEO Prerender
 * 
 * Intercepts crawler requests to /quiz/* and /article/* paths.
 * Fetches data from Supabase and serves fully-rendered HTML with
 * OG tags, structured data, and visible content.
 * 
 * Normal users get the SPA as usual (passthrough).
 */
...
export default async function handler(request: Request) {
  ...
  try {
    ...
    if (quizMatch) {
      ...
      if (!quiz) return; // passthrough — quiz not found, let SPA handle 404
